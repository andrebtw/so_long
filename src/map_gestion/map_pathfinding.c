...
/*   Updated: 2023/01/12 17:03:04 by anrodri2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../header.h"

int	pathfinding_x_y_plus(t_map_info map, char **path,
...
int	pathfinding_x_y_minus(t_map_info map, char **path,
		int *end_pos, char *allowed_chars)
{
	if (ft_strchr(allowed_chars, path[map.p_index_y - 1][map.p_index_x]))
	{
...
int	findpath(t_map_info map, char **path, int *end_pos, char *allowed_chars)
{
	...
	{
		if (pathfinding_x_y_plus(map, path, end_pos, allowed_chars) == 1)
			return (1);
		if (pathfinding_x_y_minus(map, path, end_pos, allowed_chars) == 1)
			return (1);
	}
